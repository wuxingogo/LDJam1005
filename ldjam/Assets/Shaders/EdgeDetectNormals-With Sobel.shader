...
Shader "Hidden/EdgeDetect" { 
	...
	inline fixed Luminance( fixed4 c )
    {
        return dot( c, fixed3(0.22, 0.707, 0.071) );
    }
	half Sobel(v2fSobel i)
    {
	...
	fixed4 fragSobel(v2fSobel i) :SV_Target{
	    half edge = Sobel(i);
	    fixed4 withEdgeColor = lerp(_BgColor, tex2D(_MainTex, i.uv[4]), edge);
	    //fixed4 onlyEdgeColor = lerp(_BgColor, _BackgroundColor, edge);
